...
if (user) { 
  if (user.level <= 3) {
    ...
    user.role = '🦠 NOVATO EMERGENTE';
  } else if (user.level <= 9) {
    user.role = '🦠 NOVATO AVANZADO';
  } else if (user.level <= 12) {
    user.role = '🦠 NOVATO EXPERTO';
  } else if (user.level <= 15) {
    user.role = '🦠 APRENDIZ BRILLANTE';
  } else if (user.level <= 18) {
    user.role = '🦠 APRENDIZ ILUMINADO';
  } else if (user.level <= 21) {
    user.role = '🦠 APRENDIZ DESTACADO';
  } else if (user.level <= 24) {
    user.role = '🦠 APRENDIZ EXCEPCIONAL';
  } else if (user.level <= 27) {
    user.role = '🦠 EXPLORADOR CURIOSO';
  } else if (user.level <= 30) {
    user.role = '🦠 EXPLORADOR AUDACIOSO';
  } else if (user.level <= 33) {
    user.role = '🦠 EXPLORADOR VALIENTE';
  } else if (user.level <= 36) {
    user.role = '🦠 EXPLORADOR INSPIRADOR';
  } else if (user.level <= 39) {
    user.role = '🦠 MAESTRO MAGNÍFICO';
  } else if (user.level <= 42) {
    user.role = '🦠 MAESTRO ENCANTADOR';
  } else if (user.level <= 45) {
    user.role = '🦠 MAESTRO SABIO';
  } else if (user.level <= 48) {
    user.role = '🦠 MAESTRO MAESTRO';
  } else if (user.level <= 51) {
    user.role = '🦠 GUERRERO FIERO';
  } else if (user.level <= 54) {
    user.role = '🦠 GUERRERO INDOMABLE';
  } else if (user.level <= 57) {
    user.role = '🦠 GUERRERO VIBRANTE';
  } else if (user.level <= 60) {
    user.role = '🦠 TIBURON MÁGICO';
  } else if (user.level <= 63) {
    user.role = '🦠 TIBURON RADIANTE';
  } else if (user.level <= 66) {
    user.role = '🦠 TIBURON ENCANTADA';
  } else if (user.level <= 69) {
    user.role = '🦠 REY DEL MAR';
  } else if (user.level <= 72) {
    user.role = '🦠 REY DE LA NOCHE';
  } else if (user.level <= 75) {
    user.role = '🦠 REY DEL CIELO';
    ...
    user.role = '🦠 EXPERTO EN VIRUS BOT';
  } else if (user.level <= 90) {
    user.role = '🦠 MAESTRO EN VIRUS BOT';
  } else if (user.level <= 93) {
    user.role = '🦠 GURÚ EN VIRUS BOT';
    ...
    user.role = '🦠 LEGENDARIO EN SU ESPLENDOR';
  } else if (user.level <= 108) {
    user.role = '🦠 LEGENDARIO EN SU BRILLO';
  } else if (user.level <= 111) {
    user.role = '🦠 LEGENDARIO EN SU GLORIA';
    ...
    user.role = '🦠 *QUE PRO 🙌*\n\n🥳 Eres el más activo del grupo, ¡te mereces ser administrador! 🤩🥳';
  }
}} catch (error) {
  m.error = error;
  if (error) {
